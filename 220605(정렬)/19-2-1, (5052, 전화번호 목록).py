...
class Trie:
  ...
  def insert(self, phone_num):    # 트라이 자료구조의 문자열을 삽입할 때의 알고리즘
    ...
    for num in phone_num:    # 전화번호의 글자 하나하나씩(num) 탐색해가며
      ...
      if curNode.value is True:    # 문제의 접두어 확인을 위한 알고리즘
        return False    # 현재 노드가 문자열의 끝이라면 접두어와 만난것이므로 False 반환
    curNode.value = True    # 현재 노드는 문자열의 끝을 가리킨다. 문자열의 끝 노드는 True 표시한다
...
for _ in range(int(input().rstrip())):
  ...
  tof = True    # 한 전화번호와 다른 전화번호의 접두어가 겹치는지 확인하는 변수 생성
