...
now_size = 2    # 처음 상어의 크기는 2
now_x, now_y = 0, 0

# 아기 상어의 시작 위치를 찾은 뒤에 그 위치엔 아무것도 없다고 처리
for i in range(N):
    for j in range(N):
        if array[i][j] == 9:    # 아기상어의 위치는 9로 표시
            now_x, now_y = i, j    # 아기상어의 위치에 아무것도 없다고 처리
            array[now_x][now_y] = 0
...
def bfs():
    ...
    return dist    # 이전에는 이 리턴을 while이 끝나기 전에 해서 값이 제대로 안나왔다
...
def find(dist):
    ...
    for i in range(N):
        for j in range(N):
            ...
            if dist[i][j] != -1 and 1 <= array[i][j] < now_size:    # 위에서 -1은 초기화를 의미하므로 갈 수 없다는 의미
                ############################### print("i,j",i,j)
                # 가장 가까운 물고기 1마리만 선택
...
while True:
    ...
    if value == None:
        ...
        # 현재 위치 갱신 및 이동 거리 변경
        now_x, now_y = value[0], value[1]    # find 결과 None이 아니라면 x, y 값 입력
        result += value[2]    # result에 min_dist 값 더한다
        # 먹은 위치에는 이제 아무것도 없도록 처리
        array[now_x][now_y] = 0    # 먹은 위치는 지워서 중복되지 않게 함
        ate += 1    # 먹은 후 크기 1 증가
